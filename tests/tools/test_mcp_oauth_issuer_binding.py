...
COMPOSE from upstream d9e88e19e2 into Windows ``tools.mcp_oauth`` /
``tools.mcp_oauth_provider`` owners.
"""
...
def _stored(tmp_path, issuer):
    ...
    _token_file(tmp_path).write_text(json.dumps(payload), encoding="utf-8")
...
def test_issuer_mismatch_strips_refresh_token_in_memory_and_on_disk(tmp_path):
    ...
    on_disk = json.loads(_token_file(tmp_path).read_text(encoding="utf-8"))
...
def test_matching_issuer_keeps_refresh_token_even_with_trailing_slash(tmp_path):
    ...
    assert json.loads(_token_file(tmp_path).read_text(encoding="utf-8"))["refresh_token"] == "r"
...
def test_legacy_file_without_issuer_adopts_current_issuer_once(tmp_path):
    ...
    assert (
        json.loads(_token_file(tmp_path).read_text(encoding="utf-8"))["hermes_issuer"]
