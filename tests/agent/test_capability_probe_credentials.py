...
def test_capability_paths_share_concrete_bearer(credential, expected):
    ...
    try:
        ...
        assert model_metadata._auth_headers(credential) == {
            "Authorization": f"Bearer {expected}"
        }
        assert (
            models._lmstudio_request_headers(credential)["Authorization"]
            == f"Bearer {expected}"
        )
        requests = []
        ...
        with patch(
            ...
            models.ollama_model_supports_thinking(
                "fixture", "http://localhost:11434/v1", credential
            )
...
def test_failed_callable_never_becomes_a_bearer(monkeypatch):
    ...
    for value in (failed, lambda: object(), object(), None):
        ...
        assert "Authorization" not in models._lmstudio_request_headers(value)

    url = "http://localhost:11434/v1"
    configured = {
        "base_url": url,
        "api_key": "provider-fallback",
        "extra_headers": {
            "aUtHoRiZaTiOn": "Bearer configured-fallback",
    ...
    assert (
        models._get_ollama_native_headers(url)["aUtHoRiZaTiOn"]
        == "Bearer configured-fallback"
    )
    assert (
        image_routing._resolve_inference_api_key(
            {"model": {"api_key": "model-fallback"}}, "custom"
        )
        == "model-fallback"
    )
