...
def _post_returning(monkeypatch, body: str) -> None:
    """Stub the Windows ``urlopen`` path used by ``_post_provision``.

    Upstream uses ``_json_post``; this fork still posts via ``urllib.request.urlopen``.
    Semantics under test are the response-handling contract, not the HTTP helper.
    """
...
def test_all_platforms_withheld_warns_with_the_recovery_path(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.INFO, logger="gateway.relay"):
        ok = relay.self_provision_relay()
