...
class _RelayAdapter:
    ...
    def __init__(self):
        self.handled = []
        self.handle_message = AsyncMock(side_effect=self._accept)
...
@pytest.mark.asyncio
async def test_unavailable_platform_retries_without_relay_hijack():
    """Control: a platform the relay does NOT front stays undeliverable
    (returns False for retry) — the resolver must not let relay hijack unrelated
    targets."""
