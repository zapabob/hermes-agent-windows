"""Multiplex invariant: agent-cache eviction commits memory under the OWNING profile.

``_spawn_release_thread`` used to start a bare ``threading.Thread``; threads begin with an EMPTY
context, so provider ``on_session_end`` ran under the launch profile. Downstream COMPOSE of
upstream a9838c into ``gateway/run.py`` (SR-20260913-004d); owner remains GatewayRunner (no
``run_agent_cache.py`` module split on this fork).
"""
...
def _runner(profile_homes: dict[str, Path]) -> GatewayRunner:
    ...
    runner.session_store = SimpleNamespace(
        _profile_home_for_key=lambda key: profile_homes.get(key)
    )
    return runner


def _seen_after_release(runner, key) -> dict:
    seen: dict = {}
    ...
    runner._spawn_release_thread(
        target, (None, key), f"t-{key}", inline_fallback=False, session_key=key
    )
    assert done.wait(5)
    return seen
...
def test_unscoped_housekeeping_sweep_enters_the_owning_profile_scope(tmp_path, monkeypatch):
    ...
    try:
        ...
        seen = _seen_after_release(
            _runner({"agent:b:telegram:dm:1": prof_b}), "agent:b:telegram:dm:1"
        )
    finally:
