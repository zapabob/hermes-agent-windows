"""Routed-profile policy isolation under multiplex (SR-20260913-005 / 819517fb).

Per-turn max_turns, fallback chain, hooks, aux auth.json and media policy follow
the ROUTED profile (HERMES_HOME override), not the launch-home frozen constants.
"""
            ...
            json.dumps(
                {
                    ...
                    "providers": {
                        "nous": {"agent_key": token, "access_token": token}
                    },
                }
            ),
            encoding="utf-8",
...
def test_media_policy_reads_routed_profile_config_not_env(two_homes, monkeypatch):
    ...
    (a / "config.yaml").write_text(
        yaml.safe_dump(
            {"gateway": {"strict": True, "media_delivery_allow_dirs": ["/srv/a"]}}
        ),
        encoding="utf-8",
    )
    (b / "config.yaml").write_text(
        yaml.safe_dump(
            {"gateway": {"strict": False, "media_delivery_allow_dirs": ["/srv/b"]}}
        ),
        encoding="utf-8",
    )
