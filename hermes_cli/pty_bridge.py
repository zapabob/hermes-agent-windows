...
class PtyBridge:
    ...
    def __init__(self, proc: "ptyprocess.PtyProcess"):  # type: ignore[name-defined]
        ...
        self._closed = False
        # Non-blocking master so input backpressure suspends only the owning
    ...
    def spawn(
        ...
        *,
        ...
        # Tell the child TUI it is hosted by the dashboard's xterm.js. Ink uses
        # this to skip the focus-in erase+repaint it does for native emulators
        # that coalesce hidden-tab output (xterm.js never drops frames).
    ...
    def read(self, timeout: float = 0.2) -> Optional[bytes]:
        ...
        try:
            ...
            if exc.errno in {errno.EIO, errno.EBADF}:
                return None
            # Deliberately non-blocking: readiness can disappear between
            # select() and os.read() when close/output races occur.
