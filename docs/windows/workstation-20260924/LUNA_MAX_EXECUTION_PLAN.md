...
`LM21–LM24 (T16)` のH: source worktreeは保持し、削除・再作成しない。mainへのWindows semantic portは製品commit `e310ad317f2b4a7ec651207c62f1101ad8e48372` で、六つの関連テストファイルはこの正確なSHAで157 passed / 5 skipped、Ruff 0.15.10とdiff checkも成功した。二つの読み取り専用レビューはコード範囲を承認し、gateは `NATIVE_UNQUALIFIED` を明記したローカルcommitだけを認めている。review receiptは `docs/control-mcp/IMPLEMENTATION_LOG.md` と `.omo/evidence/t16-semantic-port-gate-review.md` に記録した。実ClamAV managed DBがなくbenign/EICARを未資格のためLM23は `NATIVE_UNQUALIFIED` のまま。固定CodeGraph after-indexとcampaign-wide exact-final-head CIも未完なので、LM24 packetとrelease/push gateは未完了扱いとする。実junction race、POSIX専用child検証、プロセス間snapshot調整にも残余がある。実機のscanner/database設定、runtime、endpointには変更を加えていない。
...
2026-09-25 main follow-up: F00 started at `069eb3abd63dd5c72c7ce600a6094ce21009b580`, collected 12 nodes and passed all 12. RV01 is now also covered by `test_successful_legacy_walk_missing_sha_refuses_output`; RV02 then lacked coverage for an unborn repository HEAD. A valid-format but absent caller-supplied `integration_head` on a valid repository yields `INTEGRATION_HEAD_MISMATCH`, not `INTEGRATION_HEAD_UNKNOWN`. RV03 still lacked committed close/temp assertions. LM00 is implemented as test commit `b07bef422ef96919cd7d5814171c3455040a8197`: it runs the real frozen `inventory.generate()` path, checks the frozen and actual source digest, returned digest, and all 5,105 ledger rows. Removing row `source_sha256` with an in-memory producer mutant makes the assertion fail; the exact commit passed 13 tests and pinned Ruff 0.15.10. LM01 owner assignment remains unresolved; LM02 lifecycle assertions and mutant remain outstanding. The audit also observed two ignored pytest bytecode files after its run; their pre-run existence cannot be proven from the aggregate-only starting fingerprint, so they remain untouched.
...
2026-09-25 LM02 follow-up: main test commit `17e684bb281bc5c9e38d136209ee3f41ec3a2ed4` adds exact RV03 nodes `test_generate_closes_sqlite_and_removes_temp_dirs_on_success` and `test_generate_closes_sqlite_and_removes_temp_dirs_on_exception`. On that exact test commit, the focused nodes passed 2/2 and the full test file passed 15/15; pinned Ruff 0.15.10 and `git diff --check` passed. An in-memory `inventory.closing -> contextlib.nullcontext` mutant failed both cases at the closed-connection assertion. Independent review approved blob `18eaca82f5092a126bb4dd416fbb9bc48eed37ca`. Node 26.9.0 is rejected by the CodeGraph guard requiring Node 22, so CodeGraph was not run and no unsafe override was used. The mutant left two locked SQLite scratch directories at `C:\Users\downl\AppData\Local\Temp\workstation-inventory-db-fzws1t5e` and `C:\Users\downl\AppData\Local\Temp\workstation-inventory-db-gmoefmwv`; both were verified present and retained without cleanup. LM01 receipt/fingerprint owner assignment remains open.

2026-09-25 RV02 follow-up: test-only commit `cdffa3d7d80c907a1741f056c22b9b33ebbdfd48` adds `TestFrozenInventory::test_unborn_repository_head_refuses_inventory`. It initializes an unborn repo, exercises the real `inventory.generate()` path that cannot resolve `HEAD^{commit}`, expects `INTEGRATION_HEAD_UNKNOWN`, and asserts that no output directory is created. On the exact test tree, the full inventory test file passed 16 tests; pinned Ruff 0.15.10 and `git diff --check` passed. Independent review approved blob `f69bab9d577cd02937398abdf4b2f5afa1faa23c`. The existing invalid-format and mismatch test remains separate; this new case does not implement the missing LM01 family receipt/fingerprint validator.

2026-09-25 LM01 contract, fixed before implementation: the Integrator ledger owner in this plan is accepted as the single writer for campaign-scoped source freshness bindings. Implement `docs/windows/workstation-20260924/tools/family_receipts.py` with tests in `docs/windows/workstation-20260924/tests/test_family_receipts.py`. The expected repo-relative POSIX source scope is supplied independently by the Integrator and must never be selected from the receipt. Bind schema version, campaign/family/packet/RV, current full repository HEAD, exact sorted scope, and SHA-256 of canonical UTF-8 serialization over per-path Git status/index entry/worktree content digests. Capture must recheck HEAD and file identity/content before returning, reject malformed/duplicate/escaping paths, missing or unreadable scoped files, and symlink/reparse traversal. Unscoped and ignored temporary artifacts are excluded from hashing and must not be staged, modified, or deleted.

The receipt kind is `SOURCE_FRESHNESS_ONLY`; validation checks exact identity/scope/HEAD/fingerprint and fails closed on malformed, mismatched, or changed source. A HEAD movement invalidates the old binding even when changed files are outside scope. This digest is not a signature or tamper-proof authority: do not add a signing secret, claim semantic review completion, choose an upstream family decision, or authorize adoption/write from it. The owner makes and verifies freshness receipts; independent review and family evidence remain separate. There is no cross-process edit lease in this packet, so do not claim one.

RV02 implementation selectors: `tests/test_family_receipts.py::test_receipt_reuse_rejects_head_movement`, `::test_receipt_reuse_rejects_dirty_source_movement`, `::test_receipt_validation_preserves_unrelated_ignored_temp`, plus strict receipt/scope rejection cases. Removing the dirty-fingerprint equality check must make the dirty-source selector fail in an in-memory mutant; no source-file mutant or cleanup may touch main state or existing ignored content.
