"""Integration fixture and CLI driver for RFC 8628 MCP device-code login.
...
def run_cli(repo, mode):
    with tempfile.TemporaryDirectory(prefix="hermes-device-wire-") as directory, oauth_fixture(mode) as (base, wire):
        ...
        env.update(HOME=str(home), HERMES_HOME=str(home), PYTHONPATH=str(repo), PYTHONDONTWRITEBYTECODE="1", PYTHONUTF8="1")
        ...
        result = subprocess.run([sys.executable, "-m", "hermes_cli.main", "mcp", *command],
                                cwd=repo, env=env, stdin=subprocess.DEVNULL, capture_output=True, text=True, encoding="utf-8", timeout=40)
        ...
        if token_path.exists():
            ...
            refreshed = subprocess.run([sys.executable, "-m", "hermes_cli.main", "mcp", "test", "fixture"],
                                       cwd=repo, env=env, stdin=subprocess.DEVNULL, capture_output=True, text=True, encoding="utf-8", timeout=30)
...
if __name__ == "__main__":
    ...
    sys.stdout.write(json.dumps(run_cli(args.repo, args.mode), indent=2) + "\n")
