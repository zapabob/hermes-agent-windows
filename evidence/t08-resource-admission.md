...
Follow-up checks: `uv run --frozen --extra dev python -m pytest tests/downstream/test_resource_admission.py tests/downstream/test_windows_contracts.py -q` passed 56 tests; Ruff and ty passed on the changed test and production paths; `git diff --check` passed. The pinned command `npx --yes @colbymchenry/codegraph@1.6.0 --version` printed `1.6.0` before and after the after-sync. At code commit `3dfa896847760763cfa79af04f360f433c2a2198`, `npx --yes @colbymchenry/codegraph@1.6.0 sync .` exited 0 and reported two changed files and 95 modified nodes. The immediate status reported 8,908 files, 190,728 nodes, 610,466 edges, zero pending file changes, zero pending refs, a complete 1.6.0 index, and no worktree mismatch. The source-manifest fingerprint and impact/direct-reference results are recorded in `evidence/codegraph/T08-3dfa8968-after.json` and `evidence/codegraph/T08-3dfa8968-after-results.md`. The remaining limit is unchanged: this confirms static impact only; no real hardware calibration or T10 lifecycle integration was performed, and production admission remains inactive.
