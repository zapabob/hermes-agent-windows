...
- Standardized ACP session model application in `_apply_session_model`:
  ...
  - **Case D (Server without model switching):** Preserves server default explicitly without false claims.
...
5. `test_copilot_acp_production_path_qualification`: Full subprocess JSON-RPC server test verifying call ordering (`initialize` → `session/new` → `session/set_model` → `session/prompt`), degradation handling for unsupported models, and slug filtering.
