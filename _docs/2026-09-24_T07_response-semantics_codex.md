...
The post-correction CodeGraph sync command was later found to have invoked the global 1.5.0 CLI. The initial before/after receipts used 1.6.0, but the post-correction source is **not yet qualified by a matching 1.6.0 after-index**. A pinned 1.6.0 re-sync, status check and receipt correction remain required; the earlier unqualified sync is not counted as a passed gate.
