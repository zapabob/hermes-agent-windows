...
Product commit: `4f0b04c8d0ad54d69b586122340de7f371040d76`
...
The focused adapter regression `test_openrouter_adapter_uses_catalogue_budget_and_bounded_conditional_fetch` was run against the exact frozen predecessor and failed RED because `OpenRouterFreeRouteAdapter` was not present. On the product commit, `tests/agent/test_free_route_catalogue.py` passes with 40 tests. This includes cache-only cold reads, no-config/no-network startup, profile A→B→A isolation, same-process Desktop/Gateway sharing and reference release, GatewayRunner start/stop, Desktop lifespan, separate-process profile locking, restart during a bounded fetch, 429 handling, ETag handling, response caps, and slow body/header deadline cases.

Commands and results:

- `C:\Users\downl\Documents\New project\hermes-agent\.venv\Scripts\python.exe -m pytest tests/agent/test_free_route_catalogue.py -q` — 40 passed; one upstream `audioop` deprecation warning during the Gateway import.
...
After the product commit, CodeGraph synced 5 changed files (1,332 nodes). The product index reports 8,916 files, 191,283 nodes, and 612,407 edges, with pending changes 0, unresolved references 0, complete state, and `worktreeMismatch=null`. The pinned extraction version is current at 25, with no full reindex recommended.
...
**Q1 — `node --file downstream/delegation/free_routes.py --symbols-only`.** 87 symbols; indexed use is now the focused test file and Desktop `hermes_cli/web_server.py`.

<a id="after-q2"></a>

**Q2 — `impact FreeRouteCatalogueOwner --depth 2 --json`.** 20 nodes and 25 edges: owner methods, persistent refresh tests, `start_free_route_catalogue_refresh_host`, and Desktop `_lifespan`.

<a id="after-q3"></a>

**Q3 — `impact FreeRouteCatalogueOwner.refresh_if_due --depth 2 --json`.** 9 nodes and 8 edges: the owner method, host loop, and single-flight/persistence/refresh tests.
...
**Q6 — `impact OpenRouterFreeRouteAdapter --depth 2 --json`.** 10 nodes and 10 edges: the adapter, its methods, adapter tests, and the host factory / Desktop lifespan path.
...
The adapter registers its abort callback before DNS/connect/header/body work. Once the HTTPS socket exists, the lease can close it, and socket timeouts are clamped to the remaining total deadline. DNS resolution can block before that socket exists, so a total bound through `getaddrinfo` is not verified. The test coverage exercises trickling body data and headers after a socket exists.
