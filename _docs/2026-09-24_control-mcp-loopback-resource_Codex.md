...
After integration, four T03 CodeGraph receipts were redacted to use a stable integration-worktree identity instead of the Windows account path. Their source SHA, query, freshness and impact facts are unchanged; all four still validate against the supplied receipt schema.
